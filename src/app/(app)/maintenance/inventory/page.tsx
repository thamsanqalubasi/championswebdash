...
export default function MaintenanceInventoryPage() {
  ...
  const counts = useMemo(() => {
    return {
      ...
      value: items.reduce((sum, row) => sum + row.quantity * row.unitCost, 0),
    };
  }, [items]);
  ...
  return (
    <ModulePage
      title="Inventory"
      description="Stock levels, low-stock alerts, and usage logging."
    >
        ...
        <section className="space-y-4 rounded-lg border border-border-color bg-surface p-4">
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <article className="rounded-md border border-border-color bg-surface-elevated p-3">
              ...
              <p className="mt-1 text-xl font-semibold">{counts.lowStock}</p>
            </article>
            <article className="rounded-md border border-border-color bg-surface-elevated p-3">
              <p className="text-xs text-muted">Inventory Value</p>
              <p className="mt-1 text-xl font-semibold">{formatCurrency(counts.value)}</p>
            </article>
          </div>
          ...
          {items.length === 0 ? (
            <EmptyState
              ...
              description="Create stock items in mobile or backend first, then refresh this page."
            />
          ) : (
