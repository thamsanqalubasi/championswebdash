...
export default function ContractsPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (!cancelled) {
          setError(loadError instanceof Error ? loadError.message : "Could not load contracts.");
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  return (
    <ModulePage
      title="Contracts"
      description="Template-backed contract list, generation workflow, and PDF actions."
    >
        ...
        <section className="space-y-4 rounded-lg border border-border-color bg-surface p-4">
          <div className="grid gap-3 md:grid-cols-4">
            <article className="rounded-md border border-border-color bg-surface-elevated p-3">
              ...
              <p className="mt-1 text-xl font-semibold">{contractStats.total}</p>
            </article>
            <article className="rounded-md border border-border-color bg-surface-elevated p-3">
              <p className="text-xs text-muted">Active</p>
              <p className="mt-1 text-xl font-semibold">{contractStats.active}</p>
            </article>
            <article className="rounded-md border border-border-color bg-surface-elevated p-3">
              <p className="text-xs text-muted">Draft</p>
              <p className="mt-1 text-xl font-semibold">{contractStats.draft}</p>
            </article>
            <article className="rounded-md border border-border-color bg-surface-elevated p-3">
              <p className="text-xs text-muted">Contract Value</p>
              <p className="mt-1 text-xl font-semibold">{formatCurrency(contractStats.value)}</p>
            </article>
          </div>
          ...
          <div className="rounded-md border border-border-color bg-surface-elevated p-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <input
                ...
                className="rounded-md border border-border-color bg-surface px-3 py-2 text-sm outline-none focus:border-foreground"
              />
              <button
                type="button"
                onClick={onVerifyPin}
                disabled={pinLoading}
                className="rounded-md border border-border-color bg-surface px-3 py-2 text-sm"
              >
                {pinLoading ? "Verifying..." : "Verify PIN"}
              </button>
              <span className="text-xs text-muted">
                {pinVerified ? "Verified: delete actions unlocked" : "Not verified"}
              </span>
            </div>
          ...
          {contracts.length === 0 ? (
            <EmptyState
              ...
              description="Generate contracts in mobile or backend first, then refresh this page."
            />
          ) : (
