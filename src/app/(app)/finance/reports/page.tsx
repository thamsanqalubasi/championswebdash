...
export default function ReportsPage() {
  ...
  const maxProfit = useMemo(() => {
    ...
    return Math.max(...data.monthly.map((point) => Math.abs(point.profit)));
  }, [data]);

  return (
    <ModulePage
      title="Reports"
      description="Income statement, occupancy, expense mix, and trend dashboards will be implemented here."
    >
        ...
        <>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <article className="rounded-lg border border-border-color bg-surface p-4">
              ...
              <p className="mt-2 text-2xl font-semibold">{formatCurrency(data.summary.totalInvoiced)}</p>
            </article>
            <article className="rounded-lg border border-border-color bg-surface p-4">
              <p className="text-sm text-muted">Total Paid</p>
              <p className="mt-2 text-2xl font-semibold">{formatCurrency(data.summary.totalPaid)}</p>
            </article>
            <article className="rounded-lg border border-border-color bg-surface p-4">
              <p className="text-sm text-muted">Overdue Amount</p>
              <p className="mt-2 text-2xl font-semibold">{formatCurrency(data.summary.totalOverdue)}</p>
            </article>
            <article className="rounded-lg border border-border-color bg-surface p-4">
              <p className="text-sm text-muted">Collection Rate</p>
              <p className="mt-2 text-2xl font-semibold">{data.summary.collectionRate.toFixed(1)}%</p>
            </article>
          </div>
          ...
          <section className="grid gap-4 xl:grid-cols-3">
            <article className="rounded-lg border border-border-color bg-surface p-4 xl:col-span-2">
              ...
              {data.monthly.length === 0 ? (
                <EmptyState
                  ...
                  description="No monthly cashflow records are available yet."
                />
              ) : (
                      ...
                      <div className="flex h-40 w-full items-end rounded-md border border-border-color bg-surface-elevated p-2">
                        <div
                          ...
                          style={{
                            height: `${Math.max(
                              10,
                              (Math.abs(point.profit) / Math.max(1, maxProfit)) * 100,
                            )}%`,
                          }}
                        />
                      </div>
            ...
            <article className="rounded-lg border border-border-color bg-surface p-4">
              ...
              {data.byStatus.length === 0 ? (
                <EmptyState
                  title="No invoice statuses"
                  description="Generate invoices to populate this breakdown."
                />
              ) : (
                <ul className="space-y-3 text-sm text-muted">
                  {data.byStatus.map((item) => (
                    <li
                      key={item.label}
                      className="flex items-center justify-between rounded-md border border-border-color bg-surface-elevated p-3"
                    >
