...
export default function InvoicesPage() {
  ...
  const stats = useMemo(() => {
    ...
    const overdue = invoices.filter((row) => row.status === "overdue").length;
    const value = invoices.reduce((sum, row) => sum + row.totalAmount, 0);

    return { total, paid, overdue, value };
  }, [invoices]);

  return (
    <ModulePage
      title="Invoices"
      description="Invoice generation, status transitions, and PDF actions will be implemented here."
    >
        ...
        <section className="space-y-4 rounded-lg border border-border-color bg-surface p-4">
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <article className="rounded-md border border-border-color bg-surface-elevated p-3">
              ...
              <p className="mt-1 text-xl font-semibold">{stats.paid}</p>
            </article>
            <article className="rounded-md border border-border-color bg-surface-elevated p-3">
              <p className="text-xs text-muted">Overdue</p>
              <p className="mt-1 text-xl font-semibold">{stats.overdue}</p>
            </article>
            <article className="rounded-md border border-border-color bg-surface-elevated p-3">
              <p className="text-xs text-muted">Gross Invoiced</p>
              <p className="mt-1 text-xl font-semibold">{formatCurrency(stats.value)}</p>
            </article>
          </div>
          ...
          {invoices.length === 0 ? (
            <EmptyState
              ...
              description="Generate invoices in mobile or backend first, then refresh this page."
            />
          ) : (
