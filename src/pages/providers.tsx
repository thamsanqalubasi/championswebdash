...
export default function ProvidersPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      catch (loadError) { if (!cancelled) setError(loadError instanceof Error ? loadError.message : "Could not load providers."); }
    ...
    <ModulePage title="Providers" description="Maintainer profiles, workload stats, and provider CRUD.">
      ...
      {!loading && !error && (
        <section className="space-y-4 rounded-lg border border-border-color bg-surface p-4">
          ...
          {providers.length === 0 ? (
            <EmptyState title="No providers found" description="Create providers in mobile or backend first, then refresh this page." />
          ) : (
