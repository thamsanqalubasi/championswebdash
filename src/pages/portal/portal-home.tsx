...
import { MapPin, BedDouble, Home, Search, ChevronLeft, ChevronRight, Users, Baby } from "lucide-react";
...
type RoomListing = { id: string; display_name: string; property_name: string; type_key: string; adults_capacity: number; kids_capacity: number; total_rooms_of_type: number; price_room_only: number; price_bed_breakfast: number; price_full_board: number; photos: string[]; amenities: string[]; };
...
export default function PortalHomePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        const { data } = await supabase.from("room_type_listings").select("id,display_name,property_name,type_key,adults_capacity,kids_capacity,total_rooms_of_type,price_room_only,price_bed_breakfast,price_full_board,photos,amenities").eq("is_active", true).order("sort_order").order("created_at");
